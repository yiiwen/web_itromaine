...
$(".banner").on("click", ".banner-delete", function () {
    ...
    $(".banner-delete-confirm").click(function () {
        if (backHtml == '') {
            backHtml = $(obj).parents(".banner-action-btn").parent(".col-md-4").parent(".row").html();
        }
        $(obj).parents(".banner-action-btn").parent(".col-md-4").attr("data-delete", "1").hide();
        $("#deleteBannerModal").modal('hide');
    });
});
//移动轮播图
$(".banner").on("click", ".toLeft", function () {
    if (backHtml == '') {
        backHtml = $(this).parents(".banner-action-btn").parent(".col-md-4").parent(".row").html();
    }
    let currentObj = $(this).parents(".banner-action-btn").parent(".col-md-4");
    let leftIdx = currentObj.index();
    if (leftIdx > 0) {
        let prev = currentObj.prev();
        currentObj.after('<div class="col-md-4">' + prev.html() + "</div>");
        prev.remove();
    }
});
//移动轮播图
$(".banner").on("click", ".toRight", function () {
    ...
    if (nextObj.index() != -1) {
        nextObj.after('<div class="col-md-4">' + currentObj.html() + "</div>");
        currentObj.remove();
    }
});
...
$(".banner").on("click", ".edit", function () {
    ...
    let isMobile = $(this).parents(".banner-action-btn").prev(".banner-image").find("img").attr("data-isMobile");
    console.log(isMobile)
    $("#banner-upload-view").empty();
    $("#banner-upload-view").append(`<img src="${src}" style="width:100%;" />`);
    $("#banner-url").val(url);
    $("#banner-alt").val(alt);
    if (isMobile != 0) {$("#is-mobile").prop("checked","checked");} else {$("#is-mobile").prop("checked",false);}
    ...
    if (state == 1) {
        $("#upload-banner-success").click(function () {
            if (state == 1) {
                let isMobile = $("#is-mobile").prop("checked") ? 1 : 0;
                $(obj).parents(".col-md-4").find(".banner-image").find("img").attr("data-isMobile", isMobile);
                if (file) {
                ...
                $(obj).parents(".col-md-4").find(".banner-image").find("img").attr("data-url", $("#banner-url").val());
                $(obj).parents(".col-md-4").find(".banner-image").find("img").attr("data-alt", $("#banner-alt").val());
...
$("#save-banner").click(function () {
    ...
    $(".banner-image").each(function () {
        ...
        item['is_mobile'] = $(this).find('img').attr("data-isMobile")
        item['delete'] = $(this).parent(".col-md-4").attr("data-delete");
        item['sort'] = ++i;
        items[i - 1] = item;
    });
...
$(".banner-upload-cannel").click(function () {
    $("#uploadImg").modal("hide");
    return false;
});
